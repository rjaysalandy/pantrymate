...
const MOCK_PATIENT_RECORDS = {
  1: {
    ...
    allergies: ['Shellfish'],
    dietaryRestrictions: ['Low sugar', 'Reduced sodium'],
    currentGoal: 'Reduce weekly food waste to under 2 items and cook at home 4x per week',
    goalTarget: 2,
    nextAppointment: '2026-03-10',
    notes: 'Patient is motivated and engaged. Responds well to positive reinforcement. Struggles with impulse buying at market.',
...
const IMPORTED_RECIPES = [
  ...
  },
  {
    ...
    ingredients: [
      { item: '0.5 cup rolled oats', perServing: 0.5 }, { item: '0.5 cup milk', perServing: 0.5 }, { item: '1 tbsp honey', perServing: 1 },
    ],
...
function NutritionistDashboard({ currentUser, sharedProfiles, onLogout }) {
  ...
  const getFlags = (profile) => {
    ...
    const expiring    = profile.items.filter(i => { const d = daysUntil(i.expiryDate); return d >= 0 && d <= 3; });
              ...
              {selected && patientStats && (
                <div className="bg-white rounded-2xl shadow-lg p-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ['In Pantry', selected.items.length, 'blue'],
                      ['Save Rate', patientStats.saveRate !== null ? patientStats.saveRate + '%' : '-', 'green'],
                      ['Used',      patientStats.used,    'green'],
                      ['Wasted',    patientStats.wasted,  'red'],
                    ].map(([lbl, val, col]) => (
                  ...
                  <div className="bg-white rounded-2xl shadow-lg p-4">
                    ...
                    {panel === 'remind' && (
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mb-4">Messages appear in the patient's inbox immediately.</p>
                    ...
                    {panel === 'recipes' && (
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mb-3">Browse and send a recipe based on what is in the patient's pantry.</p>
                        {record.allergies && (
                          ...
                          {ALL_RECIPES.map(recipe => {
                            ...
                            return (
                              <div key={recipe.id} className={'border rounded-xl p-3 ' + (canMake ? 'border-green-200 bg-green-50' : 'border-gray-200')}>
                                <div className="flex items-center justify-between mb-1">
                                  <p className="font-semibold text-sm text-gray-800">{recipe.name} {recipe.id >= 101 ? '(TT)' : ''}</p>
                                ...
                                <p className="text-xs text-gray-500">{recipe.prepTime} min - {recipe.calories} cal - {recipe.difficulty}</p>
                                {!canMake && <p className="text-xs text-orange-600 mt-1">Missing: {missing.join(', ')}</p>}
                              </div>
                            );
                          })}
                        </div>
...
export default function PantryMate() {
  ...
  const matchedRecipes = ALL_RECIPES
    .map(r => {
      ...
      if (!canMake && !(missing.length <= 4 && hasNonStaple)) return null;
                    ...
                    <div key={recipe.id}
                      ...
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex gap-1 ml-2 shrink-0">
                          {isLocal && <span className="bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full text-xs font-semibold">TT</span>}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              {saveRate!==null && (
                <div className={'rounded-xl p-4 text-sm font-semibold ' + (saveRate>=70?'bg-green-50 border border-green-200 text-green-700':saveRate>=40?'bg-yellow-50 border border-yellow-200 text-yellow-700':'bg-red-50 border border-red-200 text-red-700')}>
                  {saveRate>=70 ? 'Excellent - you are using most of what you buy!'
                    : saveRate>=40 ? 'Good progress - check expiry dates regularly.'
                    : 'A lot is being wasted - try the recipe suggestions to use items up!'}
                </div>
              )}
